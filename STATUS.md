...
| v1.7-6 Longform Quality Repair Follow-up | `observation` | `.ops/validation/v1_7_6_floor3500_single_ch1_real_llm_harness.json`；`.ops/reviews/v1-7-6-floor3500-single-ch1/v1-7-6-floor3500-single-final-review/review_report.json`；`.ops/validation/v1_7_6_floor3500_4ch_final_longform.json`；`.ops/reviews/v1-7-6-floor3500-4ch/v1-7-6-floor3500-4ch-final-review/review_report.json`；`.ops/real_llm_harness/v1-7-6-floor3500-4ch-state/`；`.ops/reports/v1_7_6_quality_repair_root_cause.md`；`test_story_truth_template`；`test_immersive_mode`；`test_real_llm_demo_smoke` | 2026-05-16 已把正文长度口径统一为排除表格 / 标题 / HTML 注释，prompt 目标为正文 4200-4600，并补充 9-11 段、每段 380-520 汉字约束。单章真实 probe 已过：正文 4142 汉字，低频锚点 / 伏笔 / 短章均不阻断，刷新后的 review hard gate `should_block_next_real_llm_batch=false`；首章 opening score 高但按首章规则不阻断。按顺序恢复 4 章 / 4000 字小批后，最终 4/4 章落盘，正文分别 4797 / 3638 / 3735 / 3912 汉字，均有低频锚点与伏笔，脚本级 `passed=true`、`drift_report.status=stable`，final review hard gate 不阻断下一批。边界：第 4 章由真实 LLM 初稿 + 真实 LLM append 补长 + 手动开篇去回环修正组成，因此本项仍记为 observation，不把它升级为全自动生产许可 |
...
- **真实长篇生产化后续**：v1.7-6 单章真实 probe 已按新口径过线且 review hard gate 不阻断；按条件恢复的 4 章 / 4000 字小批已补齐 4/4 证据，final review hard gate 不阻断下一批。残余风险是 `久久` 长输出仍曾在第 4 章触发 504 / 外层终止，且第 4 章采用真实 LLM append + 手动开篇去回环修正；后续要扩大到新的全自动真实批量前，应先解决 endpoint 长输出稳定性或把补长 / 开篇修复纳入可审计自动流程。
