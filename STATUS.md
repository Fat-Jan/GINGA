...
| v1.7-6 Longform Quality Repair Follow-up | `observation` | `.ops/validation/v1_7_6_quality_repair_4000_*_real_llm_harness.json`；`.ops/reports/v1_7_6_quality_repair_4000_*_real_llm_harness.md`；`test_immersive_mode`；`test_real_llm_demo_smoke`；`.ops/validation/verify_all_v1_7_6_harness_repair.json` | 2026-05-16 尝试在 immersive runner 中对短章 / 开篇回环做最多两次质量修复，且首章只拦短章、不因起源开篇误拦；三轮 4 章 / 4000 字真实复验均 `passed=false`，第 1 章两次修复后仍低于 3500 中文字并 fail-fast，未写入章节正文；本轮只把 gate / 预算 / fail-fast 摘要修准，不允许据此扩大真实批量或声明质量修复完成 |
