...
| v1.7-5 Longform Prompt Continuity Repair | `done` | `ginga_platform/orchestrator/cli/demo_pipeline.py`；`ginga_platform/orchestrator/cli/immersive_runner.py`；`test_story_truth_template`；`test_immersive_mode`；`test_longform_quality_gate`；`test_agent_harness`；`python3 scripts/verify_all.py --quick` | 已让 `_build_chapter_prompt()` 明确消费 `workspace.CHAPTER_INPUT_BUNDLE`；bundle 的低频锚点与 hard gate 同源；immersive 同一批第 2-N 章会携带上一章生成摘要作为承接。未放宽 hard gate、未自动改旧正文、未声明真实扩大批量可用 |
...
| v2.3 Real LLM Harness | `done` | `scripts/run_real_llm_harness.py`；`test_real_llm_demo_smoke`；`test_longform_llm_smoke`；`.ops/validation/real_llm_harness.json`；`.ops/reports/real_llm_harness_report.md`；`scripts/verify_all.py` wiring | 已固定真实 LLM preflight / postflight、成本边界、隔离输出和 warn-only review gate；默认 dry-run，真实调用必须显式 `--run`；长篇小批默认 4 章、上限 5 章；v1.7-6 后正式投稿阈值改为正文汉字数口径：表格、标题、HTML 注释不计入；repair retry 计入真实调用预算（4 章最多 12 次）；fail-fast 且没有章节 artifact 时 review gate 记为 `no_chapters`，不能误读为质量通过 |
...
| v1.7-6 Longform Quality Repair Follow-up | `observation` | `.ops/validation/v1_7_6_floor3500_single_ch1_real_llm_harness.json`；`.ops/reviews/v1-7-6-floor3500-single-ch1/v1-7-6-floor3500-single-final-review/review_report.json`；`.ops/validation/v1_7_6_floor3500_4ch_final_longform.json`；`.ops/reviews/v1-7-6-floor3500-4ch/v1-7-6-floor3500-4ch-final-review/review_report.json`；`.ops/real_llm_harness/v1-7-6-floor3500-4ch-state/`；`.ops/reports/v1_7_6_quality_repair_root_cause.md`；`test_story_truth_template`；`test_immersive_mode`；`test_real_llm_demo_smoke` | 2026-05-16 已把正文长度口径统一为排除表格 / 标题 / HTML 注释，prompt 目标为正文 4200-4600，并补充 9-11 段、每段 380-520 汉字约束。单章真实 probe 已过：正文 4142 汉字，低频锚点 / 伏笔 / 短章均不阻断，刷新后的 review hard gate `should_block_next_real_llm_batch=false`；首章 opening score 高但按首章规则不阻断。按顺序恢复 4 章 / 4000 字小批后，最终 4/4 章落盘，正文分别 4797 / 3638 / 3735 / 3912 汉字，均有低频锚点与伏笔，脚本级 `passed=true`、`drift_report.status=stable`，final review hard gate 不阻断下一批。边界：第 4 章由真实 LLM 初稿 + 真实 LLM append 补长 + 手动开篇去回环修正组成，且 final review 仍有 warn-only style fingerprint issue，因此本项仍记为 observation，不把它升级为全自动生产许可 |
| v1.7-7 Style Warn Productionization | `done` + `observation` | `.ops/plans/v1-7-7-style-warn-revised-plan.md`；`.ops/subagents/v1-7-7a-codex-handoff.md`；`.ops/p7-handoff/v1-7-7a-style-warn-offline.md`；`.ops/validation/v1_7_7b_final_real_llm_harness.json`；`.ops/validation/v1_7_7b_final_retry1_real_llm_harness.json`；`.ops/validation/v1_7_7b_marker_rerun3_real_llm_harness.json`；`.ops/reports/v1_7_7b_marker_rerun3_real_llm_harness.md`；`.ops/reviews/v1-7-7b-marker-rerun3/v2-3-real-llm-review-gate/review_report.json`；`ginga_platform/orchestrator/cli/immersive_runner.py`；`scripts/run_longform_llm_smoke.py`；`test_immersive_mode`；`test_longform_failure_reporting`；`test_longform_llm_smoke`；`test_real_llm_demo_smoke` | v1.7-7a 离线代码层已完成并由主控复验：`abrupt_transition` 降为 soft warn，只报告不阻断；`generic_emotion` + `cliche_metaphor` 仍为 hard fail。v1.7-7b 首轮 / retry 暴露 `cliche_metaphor` fail-fast 与缺伏笔 marker 阻断；本轮把缺 `<!-- foreshadow:` 前移到生成质量修复 gate，并补第二轮修复长度余量、上一章 bridge 回灌、开篇回环确定性清洗和失败章号报告。修复后 `rerun3` 真实 4 章小批验收通过：脚本 `passed=true`，4/4 落盘，正文汉字数 3920 / 3527 / 5742 / 3808，均非短章；每章 `foreshadow_markers=1`；`generic_emotion=0`、`cliche_metaphor=0`；review hard gate `should_block_next_real_llm_batch=false` / `block_reasons=[]`。边界：review 仍为 warn-only，`issue_count=9`，第 1/2 章有 `系统提示` / `叮` 禁词类 observation，且 `drift_report.status=needs_review`；本项关闭 v1.7-7 DoD，但不等于全量生产许可或 v3.0 触发条件已满足 |
...
| v2.6 LLM Config | `in_progress` | `.ops/plans/v2-6-llm-config-plan.md`；`.ops/subagents/v2-6a-codex-handoff.md`；`llm_config.yaml`；`ginga_platform/orchestrator/cli/llm_config.py`；`test_llm_config`；`python -m unittest ginga_platform.orchestrator.runner.tests.test_llm_config ginga_platform.orchestrator.runner.tests.test_immersive_mode -v`；`python3 scripts/verify_all.py --quick` | v2.6a 代码层已完成并通过离线验证：项目配置、配置加载、fallback wrapper、CLI 默认 endpoint 读取配置已落位；v2.6b 真实网络 fallback 验证未做，因此整体仍不标 `done`；下一步可继续 v1.7-7a，或单独开 v2.6b 小范围 live fallback 验证 |
| v3.0 Agent Interface | `planned` | `.ops/plans/v3-0-agent-interface-plan.md` | 把 Ginga 封装为 Claude Code / Codex 通用的创作智能体：`ginga agent` 子命令层（JSON 输出）+ `GINGA_AGENT.md` contract 文件；触发条件：v1.7-7 落地 + 连续 2 轮 4 章不阻断 + 模型端稳定 + 补长/修复自动化完成；在此之前不实现 |
...
- **真实长篇生产化后续**：v1.7-7b `rerun3` 真实 4 章小批已满足脚本级 `passed=true`、无短章、无 hard style warn、无缺伏笔 marker、review hard gate 不阻断下一批。下一步若继续向 v3.0 / 更大生产边界推进，先做第二轮连续 4 章真实小批或把 warn-only 的 `系统提示` / `叮` 禁词观察项前移到生成 gate；离线 `verify_all.py --quick` 仍不证明真实长篇生产质量。
- **LLM Config 后续**：v2.6a 已完成离线代码层收口，`llm_config.yaml` 和 `llm_config.py` 已给 CLI 默认 endpoint 与 fallback wrapper 一个项目内入口；v2.6b runbook 已落 `.ops/reports/v2_6b_live_fallback_runbook.md`，但真实网络 fallback 仍未执行。当前 blocker 是缺少安全的临时 config 注入或专用 probe role，不能用 mock / dry-run 声明模型端稳定。
...
- **Story Truth Template 后续**：短章阈值已从 2400 收紧到 3500+ 中文字；v1.7-6 已把 4 章正文拉到 3500+ 且去掉连续开篇 hard gate，但 style fingerprint 仍不能稳定清零，不能直接把拆书候选、review report、market report 或 jury 原文写入 truth。
...
- **v3.0 Agent Interface**：详见 `.ops/plans/v3-0-agent-interface-plan.md`；把 Ginga 封装为 Claude Code / Codex 通用创作智能体（`ginga agent` CLI 子命令 + JSON 输出 + agent contract 文件）；当前只满足 v1.7-7 落地与 1 轮真实小批不阻断，仍缺连续第 2 轮真实批量不阻断、模型端稳定和 warn-only 禁词观察项收敛；底层接口语义未收敛前不实现。
