...
| v1.7-7 Style Warn Productionization | `in_progress` | `.ops/plans/v1-7-7-style-warn-revised-plan.md`；`.ops/subagents/v1-7-7a-codex-handoff.md`；`.ops/p7-handoff/v1-7-7a-style-warn-offline.md`；`ginga_platform/orchestrator/cli/immersive_runner.py`；`ginga_platform/orchestrator/cli/longform_policy.py`；`test_immersive_mode`；`test_story_truth_template`；`python3 scripts/verify_all.py --quick` | v1.7-7a 离线代码层已完成并由主控复验：`abrupt_transition` 降为 soft warn，只报告不阻断；`generic_emotion` + `cliche_metaphor` 仍为 hard fail，deterministic rewrite 保持清硬词。v1.7-7b 真实 4 章 LLM 验证尚未执行；应先提交 / 备份 v1.7-7a 验证切片，再开启真实小批，因此整条 v1.7-7 仍不标 `done` |
...
- **真实长篇生产化后续**：v1.7-7a 已把 style warn 分级落到离线代码层；下一步应先提交 / 备份 v1.7-7a 验证切片，再做 v1.7-7b 的 4 章真实 LLM 小批验证，且必须在隔离输出、真实调用预算和 review/hard gate 证据齐备后再判断。离线 `verify_all.py --quick` 不证明真实长篇生产质量。
