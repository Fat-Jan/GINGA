...
| v1.7-6 Longform Quality Repair Follow-up | `observation` | `.ops/validation/v1_7_6_floor3500_single_ch1_real_llm_harness.json`；`.ops/reviews/v1-7-6-floor3500-single-ch1/v2-3-real-llm-review-gate/review_report.json`；`.ops/real_llm_harness/v1-7-6-floor3500-4ch-state/`；`.ops/validation/v1_7_6_body_*_single_ch1_real_llm_harness.json`；`.ops/reports/v1_7_6_quality_repair_root_cause.md`；`test_story_truth_template`；`test_immersive_mode`；`test_real_llm_demo_smoke` | 2026-05-16 已把正文长度口径统一为排除表格 / 标题 / HTML 注释，prompt 目标为正文 4200-4600，并补充 9-11 段、每段 380-520 汉字约束。单章真实 probe 已过：正文 4142 汉字，低频锚点 / 伏笔 / 短章均不阻断，review hard gate `should_block_next_real_llm_batch=false`；首章 `opening_loop_risk` 仍为 warn。按顺序恢复 4 章小批时只落盘 3/4 章，正文分别 4797 / 3638 / 3735 汉字且均有伏笔；第 4 章多次真实调用在 `ask-llm 久久` 长输出层 504 / 外层 SIGTERM，未写 `chapter_04.md`，因此 4 章复验未完成，不能声明长篇小批恢复成功 |
...
- **真实长篇生产化后续**：v1.7-6 单章真实 probe 已按新口径过线且 review hard gate 不阻断；4 章恢复尝试只完成 3/4，当前 blocker 是 `ask-llm 久久` 第 4 章长输出 504 / 外层终止。下一步先处理 endpoint 长输出稳定性、缩短第 4 章补跑 prompt 或切换可用 endpoint，再补齐 `chapter_04.md` 并导出 4/4 review gate；在 4/4 证据完成前不得扩大批量或声明小批恢复成功。
