...
| v1.3 拆书融梗 Evidence Pipeline | `planned` | `.ops/reports/book_analysis_distillation_fusion_plan.md`；相关 jury 目录 | 尚无 `ginga_platform/book_analysis/` 实现；先做 v1.3-0 污染隔离文档与 P0 MVP |
| v1.4 BookView / explorer | `deferred` | `.ops/reports/oh_story_inspiration_roadmap.md` | 尚无 `.ops/book_views/` 或 inspect/query 实现；依赖 P2-7C 与 v1.3-0 |
...
- 新增规划路线已完成定位梳理：oh-story 参考路线、拆书融梗 / `ReferenceTropeDistillation`、BookView / explorer、review / deslop、market sidecar 已有规划与 jury 查漏补缺；当前均为 `planned` / `deferred`，不计入已完成生产能力。
...
- **v1.3 拆书融梗支线（planned）**：P2-7C 已收口；后续若推进，先做 Phase 0 文档补强和 P0 MVP（scan / split / manifest / validator / report），暂不做 D1-D12 自动化、文风指纹、Promote、Sidecar RAG，且不得进入默认 RAG 或覆盖 `StateIO` 真值。
...
- **关键红线**：`STATUS.md` 仍只作为当前状态真值；oh-story 参考路线是 planned，不计入已完成能力；`.ops/book_analysis/`、外部榜单原文、市场采集原始数据默认不得进入 RAG 或 explorer/review 输入白名单。
...
- 后续版本观察：拆书融梗规划见 `.ops/reports/book_analysis_distillation_fusion_plan.md` 与 `ROADMAP.md` §九；当前无实现代码、无验证结果，不应被描述为已完成。
