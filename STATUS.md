...
| v1.7-7 Style Warn Productionization | `planned` | `ginga_platform/orchestrator/cli/demo_pipeline.py`；`ginga_platform/orchestrator/cli/immersive_runner.py`；`.ops/validation/v1_7_6_stylegate_final4_real_llm_harness.json`；`.ops/validation/v1_7_6_stylegate_final4_retry1_real_llm_harness.json`；`test_story_truth_template`；`test_immersive_mode` | 已把 review style warn 词从“少用或不用”前置为 prompt / repair prompt 的明确禁用，并用聚焦单测覆盖；但同等 4 章 / 4000 字真实 stylegate 两次均未生成章节 artifact：第一次失败于 `style_warn abrupt_transition=2, cliche_metaphor=1`，第二次失败于 `style_warn abrupt_transition=1`。按“两次推送失败就不再纠结”规则，不继续盲目重跑；下一步应改为确定性后处理 / prompt 负样本 / 自动 rewrite 之一的窄切片，再做单章或 4 章复验 |
...
- **真实长篇生产化后续**：v1.7-6 单章真实 probe 已按新口径过线且 review hard gate 不阻断；按条件恢复的 4 章 / 4000 字小批已补齐 4/4 证据，final review hard gate 不阻断下一批。残余风险是 `久久` 长输出仍曾在第 4 章触发 504 / 外层终止，第 4 章采用真实 LLM append + 手动开篇去回环修正，且 style warn 清零在两次同等真实 stylegate 中仍失败。后续要扩大到新的全自动真实批量前，应先把补长 / 开篇修复 / style warn rewrite 纳入可审计自动流程。
- **LLM Config 后续**：v2.6a 已完成离线代码层收口，`llm_config.yaml` 和 `llm_config.py` 已给 CLI 默认 endpoint 与 fallback wrapper 一个项目内入口；真实网络 fallback 仍是 v2.6b，必须用隔离、小范围、显式真实调用验证，不能用 mock / dry-run 声明模型端稳定。
