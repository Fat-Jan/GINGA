...
| v1.7-7 Style Warn Productionization | `in_progress` | `.ops/plans/v1-7-7-style-warn-revised-plan.md`；`.ops/subagents/v1-7-7a-codex-handoff.md`；`.ops/p7-handoff/v1-7-7a-style-warn-offline.md`；`.ops/validation/v1_7_7b_final_real_llm_harness.json`；`.ops/validation/v1_7_7b_final_retry1_real_llm_harness.json`；`.ops/reports/v1_7_7b_final_retry1_real_llm_harness.md`；`.ops/reviews/v1-7-7b-style-warn-final-retry1/v2-3-real-llm-review-gate/review_report.json`；`ginga_platform/orchestrator/cli/demo_pipeline.py`；`ginga_platform/orchestrator/cli/immersive_runner.py`；`test_immersive_mode`；`test_story_truth_template`；`test_longform_quality_gate`；`python3 scripts/verify_all.py --quick` | v1.7-7a 离线代码层已完成并由主控复验：`abrupt_transition` 降为 soft warn，只报告不阻断；`generic_emotion` + `cliche_metaphor` 仍为 hard fail，deterministic rewrite 保持清硬词。v1.7-7b 已真实执行但未通过：首轮第 1 章被 `style_warn cliche_metaphor=1` fail-fast；retry 生成 4/4 章且 `drift_status=stable`、无短章、无低频锚点缺失，但第 2/3 章缺 `foreshadow_marker`，review hard gate 阻断下一批。本轮已把缺 `<!-- foreshadow:` 从后置 review 前移到生成质量修复 gate，并把 prompt / repair prompt 改为每章必须输出可机读伏笔 marker；修复后尚未重新跑真实 4 章，因此整条 v1.7-7 仍不标 `done` |
...
- **真实长篇生产化后续**：先用修复后的生成 gate 重新跑 v1.7-7b 4 章真实 LLM 小批；验收必须同时满足脚本级 `passed=true`、无短章、无 hard style warn、无缺伏笔 marker、review hard gate 不阻断下一批。离线 `verify_all.py --quick` 不证明真实长篇生产质量。
...
- **v3.0 Agent Interface**：详见 `.ops/plans/v3-0-agent-interface-plan.md`；把 Ginga 封装为 Claude Code / Codex 通用创作智能体（`ginga agent` CLI 子命令 + JSON 输出 + agent contract 文件）；触发条件是 v1.7-7 落地 + 连续 2 轮真实批量不阻断 + 模型端稳定 + 补长/修复自动化完成；底层接口语义未收敛前不实现。
