...
| v1.7-6 Longform Quality Repair Follow-up | `observation` | `.ops/validation/v1_7_6_quality_repair_4000_*_real_llm_harness.json`；`.ops/validation/v1_7_6_prompt_contract_single_ch1_real_llm_harness.json`；`.ops/validation/v1_7_6_body_*_single_ch1_real_llm_harness.json`；`.ops/reports/v1_7_6_quality_repair_root_cause.md`；`test_longform_quality_gate`；`test_immersive_mode`；`test_real_llm_demo_smoke` | 2026-05-16 先定位旧修复失败根因：模型按可见总字数压缩，旧 gate 把表格 / 标题计入中文数导致单章 `passed=true` 但正文仅 3350 汉字；随后改为正文汉字数硬口径并把 prompt 目标调到 4200-4600。新口径单章真实复验两次均 `passed=false`：一次按 4200 硬门槛失败（3340 < 4200），一次按 3600 生产底线失败（2751 < 3600），均 fail-fast 且未写章节正文；因此 4 章恢复仍冻结，不能声明质量修复完成 |
...
- **真实长篇生产化后续**：v1.7-6 单章真实探针已证明旧口径误放行：总中文数过线但正文不足；当前代码已统一正文汉字数口径并把 prompt 目标调为 4200-4600，但新口径单章仍 fail-fast（2751 < 3600）。下一步先继续收敛长度控制和修复 prompt，直到单章正文汉字数过线且 review gate 有章节可审、不阻断；此前不得恢复 4 章小批、不得扩大批量或自动改旧正文。
