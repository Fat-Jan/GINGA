#!/usr/bin/env python3
"""Validate a v1.3-3 Trope Recipe Candidate run directory."""
...
from ginga_platform.book_analysis.validation import validate_trope_recipe_run
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("run_dir", type=Path, help="trope recipe run directory under .ops/book_analysis")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = validate_trope_recipe_run(run_dir, repo_root=REPO_ROOT)
    report = {
        **report,
        "run_dir": repo_relative(run_dir),
        "validator": "ginga_platform.book_analysis.validation.validate_trope_recipe_run",
    }
    if args.json:
    ...
    print(
        f"{status} trope recipe validation: "
