#!/usr/bin/env python3
"""Build a v1.3-3 Trope Recipe Candidate sidecar run."""
...
from ginga_platform.book_analysis.corpus import build_trope_recipes


def default_run_id(source_atom_run_root: Path) -> str:
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    slug = re.sub(r"[^A-Za-z0-9_.-]+", "-", source_atom_run_root.name).strip("-") or "source"
    return f"trope-recipes-{stamp}-{slug[:40]}"
...
def build_run(args: argparse.Namespace) -> dict[str, str | int]:
    source_atom_run_root = args.source_atom_run_root
    if not source_atom_run_root.is_absolute():
        source_atom_run_root = REPO_ROOT / source_atom_run_root
    source_atom_run_root = source_atom_run_root.resolve()
    if not source_atom_run_root.is_dir():
        raise FileNotFoundError(f"source atom run directory not found: {source_atom_run_root}")

    output_root = ensure_ops_output_root(args.output_root)
    run_id = args.run_id or default_run_id(source_atom_run_root)
    ...
    run_root = build_trope_recipes(source_atom_run_root=source_atom_run_root, run_id=run_id, output_base=output_root)
    validation_report_path = Path(run_root) / "validation_report.json"
    completed = subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "scripts" / "validate_trope_recipes.py"),
            str(run_root),
            "--json",
    ...
    return {
        ...
        "recipes_path": repo_relative(Path(run_root) / "trope_recipes.json"),
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("source_atom_run_root", type=Path, help="v1.3-2 chapter atom run directory")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"FAIL trope recipe build: {exc}", file=sys.stderr)
        return 1
    status = "PASS" if result["validator_exit_code"] == 0 else "FAIL"
    print(f"{status} trope recipe build: run={result['run_dir']} validator_exit={result['validator_exit_code']}")
