...
│  - 已落地：P2 空召回诊断 + expected/relevant ids 可回归评估        │
...
### 4.6 长篇真实 LLM 批量策略（v1.7）

`久久` 30 章真实 smoke、queue 外部评审与人工裁决给出当前生产上限：

- 正式真实 LLM 长篇生成推荐一次 4 章。
- 真实 LLM 生产上限为 5 章。
- 6 章及以上只作为压力测试，不进入正式创作链默认路径。
- `ginga run --immersive` 未显式指定章节数时默认 4 章。
- CLI 对真实 LLM 多章 / 沉浸生成超过 5 章 fail-loud；mock harness 不受此限制。
- CLI 在真实 LLM 调用前执行 hard gate：最近窗口连续 2 章 `opening_loop_risk`、任一章缺低频题材锚点、或任一章缺伏笔标记时，阻断下一批真实生成。
...
v1.7 gate 已接入 `ginga review` sidecar：
...
- `hard_gate`：与生成前阻断共享检测逻辑，避免 review 口径和 CLI 口径分裂。
...
Layer 2 — 向量召回（已完成）
  ...
  - 输出：top_k 召回（按余弦相似度）
  - 当前评估索引：473 cards / 473 vectors；fallback=none
...
- ✅ RAG 真实召回质量评估
- ✅ P2 空召回诊断 + expected/relevant ids 可回归评估

### Phase 2（持续，按需推进）

| Phase 2 项 | 触发条件 | 预估投入 |
|---|---|---|
| Agent harness 补强 | 当前主线 | 离线演练 + 写入边界审计 |
| RAG 残余 blocker 小迭代 | sidecar | 守住 `recall@5 >= 0.500` |
| v1.3-1 Reference Corpus P0 MVP | 已完成，证据以 `STATUS.md` 为准 | scan / split / manifest / validator / report；输出仅限 `.ops/book_analysis/<run_id>/` |
| v1.3-2 Chapter Atom + Quality Gates | 已完成，证据以 `STATUS.md` 为准 | 结构性 `chapter_boundary` atom / quality gates / validator / report；不保存标题或原文摘录 |
...
│   └── skills/
├── .ops/
│   └── book_analysis/                    # v1.3 sidecar 污染源域；默认不进 RAG / StateIO / prompt
│       ├── contamination_check_rules.md
│       ├── p0_mvp_boundary.md
│       ├── schema/source_manifest.schema.yaml
│       └── <run_id>/                     # v1.3-1/v1.3-2 scan / split / atom / validator / report 产物
└── rag/
    ├── recall_config.yaml
