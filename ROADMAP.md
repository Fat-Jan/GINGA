...
### 9.4 v2.0 / v2.1：Harness Engineering 底座

定位：

Harness Engineering 不是继续加长 prompt，而是把 agent 的工作环境做成可读、可验证、可续接的仓库协议。v2.0 / v2.1 只负责规则地图和自检，不扩 CLI harness、不跑真实 LLM、不自动化多 agent 调度。
...
| v2.2 | CLI Harness Matrix | 扩展 `run_agent_harness.py` 覆盖 review / inspect / query / market / observability / model-topology | planned |
| v2.3 | Real LLM Harness | 固定真实 LLM preflight / postflight、成本边界、隔离输出和 review gate | planned |
| v2.4 | Multi-Agent Harness | 检查 board / task contract / provider model / 写范围 / done 归属 | planned |
| v2.5 | Stage Closeout Harness | 固定阶段收口模板、truth sync、验证摘要和 commit message 检查 | planned |

关键红线：

- v2.0 / v2.1 只证明规则地图和自检存在，不证明 CLI matrix 已扩展。
- v2.0 / v2.1 不改变 `StateIO`、真实 LLM 批量口径、默认 RAG 或多 agent 调度权限。
- 后续 v2.2 / v2.3 / v2.4 / v2.5 必须各自小切片实现并有独立验证证据。
