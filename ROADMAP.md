...
| **v1.9 / Story Truth Template** | 从原料整理小说真值模板，再用拆书产物、题材系列和长篇 drift 证据做缺口校验 | 🟢 v1.9-1 done；🟡 v1.9-2 in_progress；v1.9-3/4/5 planned | 已完成 source audit `.ops/reports/story_truth_template_source_audit.md`；下一步复核 schema / validator，只读拒绝 candidate / report / 污染路径直接进入 truth |
...
| v1.9-2 | schema 草案与 validator 设计 | 草拟 `story_truth_template` schema 与只读 validator，拒绝 candidate / report 直接进入 truth | 🟡 in_progress；worker 已交付，主控复核中 |
