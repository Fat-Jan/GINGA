...
### 9.4 v2.0 - v2.2：Harness Engineering 底座

定位：

Harness Engineering 不是继续加长 prompt，而是把 agent 的工作环境做成可读、可验证、可续接的仓库协议。v2.0 / v2.1 负责规则地图和自检，v2.2 负责离线 CLI matrix；这些仍不跑真实 LLM、不自动化多 agent 调度。
...
- v2.0 - v2.2 不改变 `StateIO`、真实 LLM 批量口径、默认 RAG 或多 agent 调度权限。
- 后续 v2.3 / v2.4 / v2.5 必须各自小切片实现并有独立验证证据。
