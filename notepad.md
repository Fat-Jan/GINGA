...
- 看板：`.ops/subagents/board.json`（以任务条目状态为准；新任务可能是 `done` / `blocked` / `in_progress`）
...
**下一步**：P2-7C provider 质量与真实 demo 已收口；v1.3-0 到 v1.3-5 拆书融梗支线、v1.4 BookView / explorer、v1.5 Review / deslop、v1.6 Market Research Sidecar、v1.7-0 到 v1.7-5 的长篇策略 / gate / prompt continuity 已完成，v1.7-6 保持 observation。v2.0-v2.5 Harness Engineering 已完成并接入 `verify_all.py` baseline。v1.7-7a Style Warn 分级离线代码层已完成；v1.7-7b 已真实尝试但未过 gate，本轮已把缺伏笔 marker 前移到生成质量修复 gate；下一步是用修复后代码重跑 v1.7-7b 的 4 章真实 LLM 小批。v2.6b 真实 fallback 验证已有 runbook，但因缺少临时 config 注入或专用 probe role 仍 blocked，不能用 mock / dry-run 当作模型端稳定证明。

**新增规划**：v1.9 Story Truth Template 已落 `.ops/plans/v1-9-story-truth-template-plan.md`。v1.9-1 source audit、v1.9-2 schema / validator、v1.9-3 `StateIO` 项目/题材契约窄切片、v1.9-4 `workspace.CHAPTER_INPUT_BUNDLE`、v1.9-5 真实小样本回归均已完成；v1.9-5 的质量结论是 observation，不是扩大批量许可。v3.0 Agent Interface 已规划（`.ops/plans/v3-0-agent-interface-plan.md`），触发条件是 v1.7-7 落地 + 模型稳定 + 连续 2 轮不阻断。
