...
- 坑点：P2-7C 只证明单章 smoke 边界，真实长篇质量必须看 `scripts/run_real_llm_harness.py --run` 的隔离证据。v1.7-7 已通过修复后 `rerun3` 真实 4 章小批：脚本 `passed=true`，无短章，无 `generic_emotion` / `cliche_metaphor` hard style warn，无缺伏笔 marker，review hard gate 不阻断；但 review 仍 warn-only（含第 1/2 章 `系统提示` / `叮` 禁词类 observation，`drift_report.status=needs_review`），所以不等于全量生产许可或 v3.0 触发条件完成。v2.3 harness 会把 repair retry 计入真实调用预算（4 章最多 12 次），并暴露 fail-fast 章节与原因；mock/dry-run 仍不证明真实生产质量。book_analysis、market 原文、review/jury/observability 报告仍默认不得进入 StateIO、RAG、prompt、raw_ideas、Foundation assets/schema 或 explorer/review 白名单。
...
**下一步**：P2-7C、v1.3-0 到 v1.3-5、v1.4、v1.5、v1.6、v1.7-0 到 v1.7-5、v1.7-7、v2.0-v2.5 已收口；v1.7-6 仍是 observation。若继续向 v3.0 / 更大生产边界推进，先做第二轮连续 4 章真实小批或把 warn-only 的 `系统提示` / `叮` 禁词观察项前移到生成 gate。v2.6b 真实 fallback 验证已有 runbook，但因缺少临时 config 注入或专用 probe role 仍 blocked，不能用 mock / dry-run 当作模型端稳定证明。

**新增规划**：v1.9 Story Truth Template 已落 `.ops/plans/v1-9-story-truth-template-plan.md`。v1.9-1 source audit、v1.9-2 schema / validator、v1.9-3 `StateIO` 项目/题材契约窄切片、v1.9-4 `workspace.CHAPTER_INPUT_BUNDLE`、v1.9-5 真实小样本回归均已完成；v1.9-5 的质量结论是 observation，不是扩大批量许可。v3.0 Agent Interface 已规划（`.ops/plans/v3-0-agent-interface-plan.md`），当前仍缺连续第 2 轮真实批量不阻断、模型端稳定和 warn-only 禁词观察项收敛。
