...
**新增规划**：v1.9 Story Truth Template 已落 `.ops/plans/v1-9-story-truth-template-plan.md`。v1.9-1 source audit 已落 `.ops/reports/story_truth_template_source_audit.md`，v1.9-2 schema / validator 已落 `foundation/schema/story_truth_template.yaml` 与 `scripts/validate_story_truth_template.py`，v1.9-3 已让 `ginga init` 经 `StateIO` 写 `locked.PROJECT_CONTRACT` / `locked.GENRE_CONTRACT`；下一步 v1.9-4 构建章节输入包。
